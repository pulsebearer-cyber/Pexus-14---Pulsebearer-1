...
import { SITEMAP_DATA } from '../src/data/sitemapData';
...
const DOMAIN = process.env.VITE_SITE_URL || 'https://pexusghana.com';
...
async function generateSitemap() {
  ...
  // 5. Dynamic parts routes from Parts.tsx
  ...
  const getPriority = (url: string) => {
    ...
    if (url === '/repairs' || url === '/parts' || url === '/blog' || url === '/service-areas') return '0.85';
    if (url.startsWith('/parts/') || url.startsWith('/blog/')) return '0.8';
